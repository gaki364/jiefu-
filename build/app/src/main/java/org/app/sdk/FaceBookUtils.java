...
public class FaceBookUtils {
    ...
    public void loginFacebook(Activity activity, loginCallbackInterfact _cb) {
        Log.d(m_actTag, "loginFacebook");
        FaceBookUtils.loginCallback = _cb;
        LoginManager.getInstance().logInWithReadPermissions(m_activity, Arrays.asList("public_profile"));
    }

    public void logoutFacebook(Activity activity, loginCallbackInterfact _cb) {
        Log.d(m_actTag, "logoutFacebook");
        FaceBookUtils.loginCallback = _cb;
...
//        parameters.putString("fields", "id,name,link,email,first_name,last_name,gender,picture,locale,timezone,updated_time,verified");
    ...
    public void nativeToLogic(Map<String, String> map) {
        Log.d(m_actTag, "nativeToLogic" + map.toString());
        if (FaceBookUtils.loginCallback != null) {
            FaceBookUtils.loginCallback.onCallback(map.toString());
            FaceBookUtils.loginCallback = null;
        }
    }
...
//                    String[] sourceStrArray = avatarUrl.split("\\?");
                    ...
                    Log.e(m_actTag, "get facebook photo fail 4" + e.getStackTrace().toString());
                }
            }
        }
        );
...
//                getFacebookUserPictureAsync(loginResult.getAccessToken().getUserId());
            ...
            public void onCancel() {
                Log.d(m_actTag, "facebook login cancel");
                Map<String, String> map = new HashMap<String, String>();
                map.put("result", "2");
            ...
            public void onError(FacebookException error) {
                Log.d(m_actTag, "facebook login error:" + error.toString());
                Map<String, String> map = new HashMap<String, String>();
                map.put("result", "3");

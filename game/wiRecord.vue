		...
		<view class="bg cardBg">
			...
			<view style="font-size: 20rpx; padding-top: 10rpx;">
				<block v-for="(item, index) in list" v-bind:key="index">
					<view class="hengzhe" style="margin: auto; margin-top: 10rpx; justify-content:space-around; background: #000000; border-radius: 10rpx; width: 90%; padding-top: 10rpx; padding-bottom: 10rpx; height: 46rpx; position: relative;">
						<view style="width: 33%; margin: auto; margin-left: 10rpx;">{{item.Date}}</view>
						<view style="width: 26%; white-space:normal; text-align: center; text-overflow: ellipsis; margin: auto; color: #F2F65D;">{{gHelper.formatMoney(item.Amount)}}</view>
						<view style="width: 40%; text-align: center; color: #FFFFFF; display: flex; margin: auto;" v-if="item.Status == 0">
						...
						<view style="width: 40%; text-align: center; color: #FFFFFF; display: flex; margin: auto;" v-if="item.Status == 1">
							<view style="margin: auto 10rpx auto auto;">
						...
						<view style="width: 40%; text-align: center; color: #FFFFFF; display: flex; margin: auto;" v-if="item.Status == 2">
							<view style="margin: auto 10rpx auto auto;">
						...
						<view style="width: 40%; text-align: center; color: #FFFFFF; display: flex; margin: auto;" v-if="item.Status == 22">
							<view style="margin: auto 10rpx auto auto;">
								<image :src="gAssets.statusImg2" style="width: 28rpx; height: 31rpx;"></image>
							</view>
							<view style="margin: auto auto auto 0rpx; font-size: 20rpx; width: 70%;">
						...
						<view class="helpBtn" v-if="item.Status == 2 || item.Status == 22" @click="onButtonClick_showPop(item.Status)"></view>
					</view>
				</block>
			</view>
		...
		<view class="MaskWindow" v-if="showPop">
			<view class="out">
				<view class="window">
					...
					<view class="centerBg">
						<view style="height: 20rpx;"></view>
						<view style="width: 94%; margin: auto; color: #FFFFFF; padding-bottom: 20rpx; border-radius: 10rpx; font-size: 26rpx; white-space: pre-wrap;">{{popTest}}</view>
...
<script>
	...
	export default {
		...
		methods: {
			flush() {
				uni.stopPullDownRefresh();
				this.gRequest.infomationManager.withdrawRecords(this.onEventCallback_withdrawRecords.bind(this));
			},
